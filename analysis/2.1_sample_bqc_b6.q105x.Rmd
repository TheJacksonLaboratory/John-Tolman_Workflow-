---
title: "Step 1: Sample QC [b6.q105x]"
...
filepaths = '/Users/corneb/Documents/MyJax/CS/Projects/Simon.John.Tolman/haplo.reconst/2020-11-11/b6.q105x/all_samples_sorted/'
#setwd(filepaths)
...
gm <- read_cross2("/Users/corneb/Documents/MyJax/CS/Projects/Simon.John.Tolman/haplo.reconst/2020-11-11/b6.q105x/all_samples_sorted/qtl2/gm_sorted.json")
...
#save into pdf
pdf(file = "output/Percent_missing_genotype_data_b6.q105x.pdf", width = 20, height = 20)
...
save(percent_missing,file = "data/percent_missing_id_b6.q105x.RData")
...
There are 16 mice that are missing >10% of their genotypes. These samples are:
...
bad.sample.sex <- subset(samples, (samples$Inferred.Sex != samples$Sex) | is.na(samples$Sex))
...
save(summary.cg,file = "data/summary.cg_b6.q105x.RData")
...
project <- read.csv("/Users/corneb/Documents/MyJax/CS/Projects/Simon.John.Tolman/haplo.reconst/2020-11-09/all.samples/all.samples/project_b6.q105x.csv", as.is=T)
...
#load("data/pr.RData")
pr<-readRDS("/Users/corneb/Documents/MyJax/CS/Projects/Simon.John.Tolman/haplo.reconst/2020-11-11/b6.q105x/all_samples_sorted/Jax.Columbia_John__GigaMUGA_genoprobs_36state_sorted.rds")
#load("data/m.RData")
#load("data/nxo.RData")
...
#str(pr)
save(m, file = "data/m_b6.q105x.RData")
save(nxo, file = "data/nxo_b6.q105x.RData")

e <- calc_errorlod(gm, pr)
#str(e)
e <- do.call("cbind", e)
save(e, file = "data/e_b6.q105x.RData")

#snpg <- predict_snpgeno(gm, m)
#snpg <- do.call("cbind", snpg)
#save(snpg, file = "data/snpg_b6.q105x.RData")
...
pdf(file = "output/number_crossover_b6.q105x.pdf")
...
iplot(seq_along(errors_ind), errors_ind, indID=lab,
      chartOpts=list(xlab="Mouse", ylab="Percent genotyping errors", ylim=c(0, 8),
                     axispos=list(xtitle=25, ytitle=50, xlabel=5, ylabel=5)))
save(errors_ind, file = "data/errors_ind_b6.q105x.RData")
...
#pdf(file = "output/Percent_genotype_errors_obs_vs_predicted_b6.q105x.pdf",width = 20, height = 20) 
#par(pty="s")
#err_direct <- rowMeans(snpgn != gobs, na.rm=TRUE)*100
#errors_ind_0 <- rowSums(e > 0)[rownames(gm$covar)]/n_typed(gm)*100
#par(mar=c(4.1,4.1,0.6, 0.6))
...
bad.sample <- qc_info[qc_info$generation ==1 | qc_info$Number_crossovers <= 200 | qc_info$Number_crossovers >=1000 | qc_info$percent_missing >= 10 | qc_info$remove.id.duplicated == TRUE,]

save(qc_info, bad.sample, file = "data/qc_info_bad_sample_b6.q105x.RData")
...
save(gm_samqc, file = "data/gm_samqc_b6.q105x.RData")
...
#save(e,g,snpg, file = "data/e_g_snpg_samqc_b6.q105x.RData")
save(e,g, file = "data/e_g_snpg_samqc_b6.q105x.RData")
...
names(removed.ids) <- "Mouse.ID"
removed.ids$Neogen_Sample_ID <-  as.character(do.call(rbind.data.frame, strsplit(as.character(removed.ids$Mouse.ID), "_"))[,6])
...
bad$diff_sex <- ifelse(bad.sample$sex.match ==TRUE | is.na(bad.sample$sex.match) == TRUE, 'XX', '')
...
badind[] %>% 
   mutate(
     ...
     #             ''),
     DO_generation = ifelse(DO.Generation == 'XX',
                  cell_spec(DO.Generation, color = 'light blue',background = 'ligh blue'),
                  ''),
